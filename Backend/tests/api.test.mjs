...
describe('Categories API', () => {
  ...
  it('rejects a duplicate category name with 409', async () => {
    const res = await request(state.base, 'POST', '/categories', SEED_PAYLOAD('Food'))
    assert.equal(res.status, 409)
    assert.match(res.data.message, /already exists/)
  })

  it('rejects a create without a name with 400', async () => {
    const res = await request(state.base, 'POST', '/categories', { icon: '🧪', color: '#111111' })
  ...
  it('gets an existing category by id', async () => {
    const categories = await getCategories(state.base)
    const res = await request(state.base, 'GET', `/categories/${categories[0].id}`)
    assert.equal(res.status, 200)
    assert.equal(res.data.data.id, categories[0].id)
  })

  it('returns 404 for a nonexistent category', async () => {
    const res = await request(state.base, 'GET', '/categories/999999')
    assert.equal(res.status, 404)
    assert.equal(res.data.message, 'Category not found.')
  })

  it('updates a category', async () => {
    const created = await request(state.base, 'POST', '/categories', SEED_PAYLOAD('TempCat'))
  ...
  it('returns 404 when updating a nonexistent category', async () => {
    const res = await request(state.base, 'PUT', '/categories/999999', SEED_PAYLOAD('X'))
    assert.equal(res.status, 404)
  })

  it('deletes an unused category and returns its id', async () => {
    const created = await request(state.base, 'POST', '/categories', SEED_PAYLOAD('ToDelete'))
    const res = await request(state.base, 'DELETE', `/categories/${created.data.data.id}`)
    assert.equal(res.status, 200)
    assert.equal(res.data.data.id, created.data.data.id)
  })

  it('blocks deleting a category used by a transaction with 409', async () => {
    const category = (await request(state.base, 'POST', '/categories', SEED_PAYLOAD('InUse'))).data.data
    ...
    const res = await request(state.base, 'DELETE', `/categories/${category.id}`)
    assert.equal(res.status, 409)
  ...
  it('blocks deleting a category used by a budget with 409', async () => {
    const category = (await request(state.base, 'POST', '/categories', SEED_PAYLOAD('InUse2'))).data.data
...
describe('Transactions API', () => {
  ...
  it('creates a transaction and returns 201 with stringified amount', async () => {
    const cat = (await getCategories(state.base)).find((c) => c.name === 'Food')
  ...
  it('validates required fields and value ranges', async () => {
    ...
    for (const [payload, message] of cases) {
      const r = await request(state.base, 'POST', '/transactions', payload)
  ...
  it('filters by type, search, category, and date range', async () => {
    ...
    const incomeOnly = await request(state.base, 'GET', '/transactions?type=INCOME')
    assert.equal(incomeOnly.data.data.length, 1)
    assert.equal(incomeOnly.data.data[0].type, 'INCOME')

    const search = await request(state.base, 'GET', '/transactions?search=market')
    assert.equal(search.data.data.length, 1)
    assert.equal(search.data.data[0].description, 'Market run')

    const byCat = await request(state.base, 'GET', `/transactions?categoryId=${food.id}`)
  ...
  it('sorts by amount asc/desc and rejects invalid sort keys', async () => {
    const asc = await request(state.base, 'GET', '/transactions?sortBy=amount&sortOrder=asc')
    const amountsAsc = asc.data.data.map((t) => Number(t.amount))
    assert.deepEqual(amountsAsc, [...amountsAsc].sort((a, b) => a - b))

    const desc = await request(state.base, 'GET', '/transactions?sortBy=amount&sortOrder=desc')
    const amountsDesc = desc.data.data.map((t) => Number(t.amount))
    assert.deepEqual(amountsDesc, [...amountsDesc].sort((a, b) => b - a))

    const badSort = await request(state.base, 'GET', '/transactions?sortBy=category')
    assert.equal(badSort.status, 400)
    assert.equal(badSort.data.message, 'sortBy must be date or amount.')

    const badOrder = await request(state.base, 'GET', '/transactions?sortOrder=sideways')
  ...
  it('paginates with correct meta', async () => {
    const total = 5
    const res = await request(state.base, 'GET', '/transactions?page=1&limit=2')
    ...
    const page3 = await request(state.base, 'GET', '/transactions?page=3&limit=2')
    assert.equal(page3.data.data.length, 1)
    assert.equal(page3.data.meta.page, 3)
  })

  it('rejects invalid pagination parameters', async () => {
    const badPage = await request(state.base, 'GET', '/transactions?page=0')
    assert.equal(badPage.status, 400)
    assert.equal(badPage.data.message, 'page must be at least 1.')

    const badLimit = await request(state.base, 'GET', '/transactions?limit=101')
  ...
  it('gets, updates, and deletes a transaction', async () => {
    ...
    const got = await request(state.base, 'GET', `/transactions/${id}`)
    ...
    const deleted = await request(state.base, 'DELETE', `/transactions/${id}`)
    assert.equal(deleted.status, 200)
    assert.equal(deleted.data.data.id, id)

    const missing = await request(state.base, 'GET', `/transactions/${id}`)
...
describe('Budgets API', () => {
  ...
  it('validates month, year, and amount', async () => {
    ...
    for (const [payload, message] of cases) {
      const r = await request(state.base, 'POST', '/budgets', payload)
      assert.equal(r.status, 400, JSON.stringify(payload))
      assert.equal(r.data.message, message, JSON.stringify(payload))
    }
  })
  ...
  it('computes On Track / Near Limit / Over Budget progress from real spending', async () => {
    ...
    const fresh = await request(state.base, 'GET', `/budgets/${budgetId}`)
    ...
    const nearLimit = await request(state.base, 'GET', `/budgets/${budgetId}`)
    ...
    const over = await request(state.base, 'GET', `/budgets/${budgetId}`)
  ...
  it('filters budgets by month and year', async () => {
    ...
    const filtered = await request(state.base, 'GET', `/budgets?month=3&year=2025`)
  ...
  it('updates a budget amount and recomputes status', async () => {
    ...
    assert.equal(updated.status, 200)
    const enriched = await request(state.base, 'GET', `/budgets/${id}`)
  ...
  it('deletes a budget and returns 404 for a nonexistent budget', async () => {
    const bills = (await getCategories(state.base)).find((c) => c.name === 'Bills')
    const created = await request(state.base, 'POST', '/budgets', {
      categoryId: bills.id,
      month: curM,
      year: curY,
    ...
    const id = created.data.data.id
    const deleted = await request(state.base, 'DELETE', `/budgets/${id}`)
    assert.equal(deleted.status, 200)
    assert.equal(deleted.data.data.id, id)

    const missing = await request(state.base, 'GET', `/budgets/${id}`)
    assert.equal(missing.status, 404)
...
describe('Dashboard API', () => {
  ...
  it('returns zeroed summary for an empty database', async () => {
    const res = await request(state.base, 'GET', '/dashboard/summary')
    assert.equal(res.status, 200)
...
describe('Reports API', () => {
  ...
  it('aggregates 12 months with deltas and null delta for the first month', async () => {
    ...
    const res = await request(state.base, 'GET', '/reports/monthly')
    assert.equal(res.status, 200)
  ...
  it('ranks expense categories by total and reports the highest', async () => {
    ...
    const res = await request(state.base, 'GET', '/reports/categories')
    assert.equal(res.status, 200)
